...
if [ "$SETUP_ONLY" = true ]; then
    ...
    echo "  │                                                          │"
    echo "  │  例: 手動起動（config/settings.yaml の各役CLIに従う）    │"
    echo "  │  source ./lib/cli_adapter.sh                             │"
    echo "  │  tmux send-keys -t shogun:main \"\$(build_cli_command shogun)\" Enter │"
    echo "  │  tmux send-keys -t multiagent:agents.${PANE_BASE} \"\$(build_cli_command karo)\" Enter │"
    echo "  │                                                          │"
    echo "  │  全9ペインは役ごとにコマンドが異なるため、一括起動は     │"
    echo "  │  上記の通り ./shutsujin_departure.sh（-s なし）を推奨    │"
    echo "  └──────────────────────────────────────────────────────────┘"
    echo ""
fi
